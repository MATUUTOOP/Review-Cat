...
- Workers should stay in sync with `main` to pull the latest engrams into their worktrees.
	- In the dev harness, the Director detects drift via worker heartbeats (e.g., `catalog_hash_seen`) and can require sync via agent-bus messages (see `docs/specs/dev/systems/AgentBusSystem.md`).
